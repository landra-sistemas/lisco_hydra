...
class HydraServer extends Server {
  async initialize() {
    ...
    await hydra.registerService(); //Sobreescribir la app para que la configuracion del server se realice normalmente.

    this.app = await hydra.integration.getService(); //Save for later use

    this.hydra = hydra; //Continuar con la inicializacion normalmente
...
class HydraClusterServer extends ClusterServer {
  ...
  async initUnclustered() {
    await this.server.initialize(); //Ignorar el arranque del server y realizar solo la config. Se hace automáticamente en hydra.
